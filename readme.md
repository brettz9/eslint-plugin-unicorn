...
```json
{
	...
	"eslintConfig": {
		...
		"rules": {
			...
			"unicorn/prefer-match-all": "error",
...
- [prefer-match-all](docs/rules/prefer-match-all.md) - Prefer `for (match of str.matchAll(regexp))` over `while ((match = regexp.exec(str)) !== null)`. *(fixable)*
